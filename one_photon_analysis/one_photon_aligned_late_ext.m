activity = readtable('Z:\Hrishikesh Bhagwat\L-Ext_activity_HB.xlsx');
[~, header] = xlsread('Z:\Hrishikesh Bhagwat\L-Ext_activity_HB.xlsx');
cells = str2double(header(:,3:end));

group_info = readtable('Z:\Olena Bukalo\AS project\AS data\neurons\neurons 1p\final\E-Ext CS_responsivity.xlsx','Sheet','auc_diff_obs');

freezing_folder = 'E:\AS-Gq-hSyn-GRIN\CNO-VEH_extinction\FreezingOutput_processed';
freeze_threshold = 30;
move_threshold = 30;
FPS = 30;
...
animalNames = {
    ...
    'AS-Gq-hSyn-GRIN-7';
    'AS-Gq-hSyn-GRIN-8';
    % 'AS-Gq-hSyn-GRIN-9'; % not in E-Ext CS_responsivity.xlsx
    'AS-Gq-hSyn-GRIN-11';
    'AS-Gq-hSyn-GRIN-12';
    'AS-Gq-hSyn-GRIN-13';
    % 'AS-Gq-hSyn-GRIN-14'; % not in E-Ext CS_responsivity.xlsx
    'AS-Gq-hSyn-GRIN-15';
    'AS-Gq-hSyn-GRIN-17';
    'AS-Gq-hSyn-GRIN-18';
    'AS-Gq-hSyn-GRIN-20';
...
for group=1:numel(groupNames)
    for i=1:num_CS
        ...
        title(sprintf('CS %d (%s)', i+45, groupNames{group}));
    end
end
...
for group=1:numel(groupNames)
    ...
    figure;
    plot(linspace(-30,30,length(ts1{1})), mean_mean_trial_traces, 'Color', [0 0.4470 0.7410], 'LineWidth', 2); hold on;
    ...
    for score=1:num_scores
        ...
        color(color_idx) = 0.1;
        plot(linspace(-30,30,length(ts1{1})), color, 'LineWidth', 3, 'Color', cmap(possible_scores(score)+(numAnimals{group}*num_CS)+1,:), 'Marker', '_'); hold on;
    end
    xline(0,'-',{'CS'});
    colorbar;
    clim([-numAnimals{group}*num_CS, numAnimals{group}*num_CS]);
    title(sprintf('CS 46-50 (%s)', groupNames{group}));
end
...
function [signal_mean, signal_sem, signal_combined_mean, signal_sem_combined_sem] = freeze_move_plot(freeze_move_list,animalNames,...
    ...
    for group=1:numel(groupNames)
        for CS=1:num_CS
            ...
            xline(0,'-',plot_label);
            title(sprintf('CS %d (%s)', CS+45, groupNames{group}));
        end
    end
